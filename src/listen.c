...
int start_listen(struct tsserv_config* cfg, struct tssparent_context* ctx)
{
	...
	if(cfg->http_port != NULL) {
		ctx->evhttp = evhttp_new(ctx->ev_base);
		evhttp_bind_socket(ctx->evhttp, cfg->http_host, strtol(cfg->http_port, NULL, 10));
		evhttp_set_cb(ctx->evhttp, cfg->http_path, http_connect_cb, ctx);
	}
